...
# Env-var overridable thresholds (tune without redeploy)
ML_THRESHOLDS = {
...
if AGGRESSIVE_PAPER:
    ...
    logging.getLogger("signal_aggregator").info(
        "ML thresholds: SD=%d AN=%d WT=%d", ML_THRESHOLDS["speed_demon"],
        ML_THRESHOLDS["analyst"], ML_THRESHOLDS["whale_tracker"]
    )
        ...
        while True:
            try:
                ...
                try:
                    ...
                    # Also track in simple today key for dashboard
                    await redis_conn.hincrby("filter:stats:today", "total_signals", 1)
                    await redis_conn.expire("filter:stats:today", 86400)
                except Exception:
                    pass
