...
**Outstanding V5A blockers (3):**

- [ ] **PC1 — `~5 SOL on trading wallet`** (Jay action). On-chain balance **0.064095633 SOL** (re-verified Helius `getBalance` 2026-05-20 by `PC1-WALLET-TARGET-RECONCILE-001`; unchanged since 2026-04-21). **Top-up target updated 2026-05-20 from prior `≥1.5-2.5 SOL` to `~5 SOL`** to honor the V5A trial budget per **D-S3 of `docs/findings/V5A_GO_LIVE_DECISIONS.md`** (daily realized-loss halt 1.5 SOL = 30% of budget; cumulative realized-loss halt 3.0 SOL = 60% of budget; leaves a ~2 SOL operational floor at the cumulative halt). The earlier 1.5-2.5 SOL target reflected the *operational minimum* for the swap router (`MIN_POSITION_SOL=0.05` × `MAX_POSITION_SOL_FRACTION=0.10` (code default) = effective ≥0.05 SOL per position) and remains true as a lower bound; the trial budget is the binding target and supersedes it because D-S3 governs the trial. Re-verify at flip time via Helius MCP. Same blocker that 2026-05-01 V5A-GO-NO-GO PC1 flagged. **Related (do not fix here):** PC4's pre-flip check notes `DAILY_LOSS_LIMIT_SOL` currently set to **4.0** on bot_core (see §2 row); D-S3 specifies a daily halt at **1.5 SOL** realized loss. These should be reconciled — the V5A flip session will set `DAILY_LOSS_LIMIT_SOL=1.5` on Railway and update PC4 accordingly. Out of scope for this docs-only fix (env change + verification belong with the flip).
