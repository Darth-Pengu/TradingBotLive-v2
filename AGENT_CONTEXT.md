...
| **BOT_CORE_FILL_MC_CEILING_USD** | **1000** | **RETUNED (NO-MOMENTUM-90S-FILTER-RETUNE-DEPLOY-001, 2026-05-13 03:29:21Z UTC; container restart 03:38:37Z UTC).** Originally deployed at **3000** by STOP-LOSS-20-RUG-FILTER-DEPLOY-001 (2026-05-11). Retuned to **1000** per NO-MOMENTUM-90S-AUDIT-001 §7 C1 (audit `docs/audits/NO_MOMENTUM_90S_AUDIT_001_2026_05_12.md`). Fill-time market-cap ceiling, env-controlled. Reads inside `services/paper_trader.paper_buy` after `entry_price` is computed; if `entry_price * 1_000_000_000 > ceiling`, returns `{success: False, error: "fill_mc_ceiling_exceeded"}` and increments Redis counter `bot:filter:fill_mc_ceiling:rejects:<date>` (14d TTL). Default 0 in code = disabled. Closes both `stop_loss_20%` rug-tier bleed (originally targeted at $3K) AND `no_momentum_90s` $1k-$3k MC-band bleed (this retune). Counterfactual at retune: marginal +1.49 SOL/day (W3+W4 8.12d), +3.02 SOL/day (W4-only); 0 FP winners (max TRAILING_STOP-win MC in sample = $892). Rollback: env→3000 (no redeploy required). First post-deploy log: `FILL_MC_CEILING reject: 6X5V79NvN85P mc=$10753 > ceiling=$1000` at 03:41:38Z confirms plumbing. |
...
**Outstanding V5A blockers (4):**

- [ ] **PC1 — `~3 SOL transfer to trading wallet`** (Jay action). On-chain balance **0.064095633 SOL** (verified Helius `getBalance` 2026-05-14 ~12:57 UTC, unchanged since 2026-04-21). Top-up target ≥1.5-2.5 SOL so `MIN_POSITION_SOL=0.05` × `MAX_POSITION_SOL_FRACTION=0.10` (code default) yields effective ≥0.05 SOL per position; without it the swap router rejects. Re-verify at flip time via Helius MCP. Same blocker that 2026-05-01 V5A-GO-NO-GO PC1 flagged.

- [ ] **PC2 — Post-C1 observation through combined eval ≥2026-05-27.** Current SD-paper config has changed materially since the old "24-48h post Sessions A-D" anchor — latest behavioural deploy is `NO-MOMENTUM-90S-FILTER-RETUNE-DEPLOY-001` (C1, `BOT_CORE_FILL_MC_CEILING_USD=3000→1000` at 2026-05-13 03:38:37Z UTC). The observation window for V5A is therefore "from C1 deploy through the combined `STOP-LOSS-20-RUG-FILTER-EVAL-001` + `NO-MOMENTUM-90S-EVAL-001` at ≥2026-05-27" (+14d from C1 per ROADMAP Decision Log 2026-05-13). V5A cannot flip until the combined eval has run and its verdict supports continuing. Verify via STATUS.md + ROADMAP Decision Log at flip time.

- [ ] **PC3 — `LIVE-MODE-FILTER-PARITY-001-V2` landed** (NEW Tier-1 🟡, added 2026-05-14 per audit `docs/audits/LIVE_MODE_FILTER_PARITY_001_2026_05_14.md` §8.2 open question). Without V2, a live relaunch reintroduces the $1k–$3k fill-time MC bleed that C1 just eliminated on the paper path. SA's signal-time `SD_MC_CEILING_USD=3000` is NOT a substitute — it is the gate C1 was built to backstop. Recommended scope (Option A from the audit): gate in `services/bot_core.py` live branch *before* `execute_trade` using existing `self._get_token_price(mint)`, mirroring `paper_buy`'s fill-time MC + env var + reject-log + Redis-counter. Needs explicit Jay authorization. Re-verify post-deploy via `grep BOT_CORE_FILL_MC_CEILING_USD services/bot_core.py` + on-chain reject-log instrumentation.

- [ ] **PC4 — V5A flip itself: `TEST_MODE=false` on bot_core**. Terminal action per CLAUDE.md "Live trading mode — session-gated" rule (§Operating Principles). Pre-flip self-check at flip time:
  - PC1-PC3 all satisfied.
  - `bash scripts/live_flip_prep.sh` ran clean (per CLAUDE.md "Live-flip pre-flight procedure" / CLEAN-003).
  - `market:mode:current=NORMAL` at flip time (verified Redis `get market:mode:current`). At session start it is currently NORMAL automatically — no manual override needed unless the automated calculation lands non-NORMAL. If non-NORMAL at flip time, optionally set `market:mode:override=NORMAL EX 86400` until the window stabilises (this replaces the old "renew Redis daily TTLs" item).
  - `DAILY_LOSS_LIMIT_SOL` on bot_core ≥ wallet floor (currently 4.0).
  - On-chain balance within 0.01 SOL of `portfolio_snapshots.total_balance_sol`.
  - Sell-storm circuit breaker present (`SELL_FAIL_THRESHOLD` < 1000).

**Related milestones (parallel work, not V5A preconditions):**

- `ML_THRESHOLD_RETUNE_002` (≥2026-05-19, ≥7d post-`BOT_CORE_ML_GATE` clean data). Paper-only at flip per ML-SCORE-ATH-VALIDATION-001 STATUS 2026-05-12 — orthogonal to V5A.
