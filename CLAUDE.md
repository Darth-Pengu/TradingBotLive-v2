...
| `docs/findings/MARKET_REGIME_GAP.md` | Market mode is **volumetric** (3 ANDed legs: pumpfun_vol/grad_rate/dex_vol), not CFGI-driven; the **HIBERNATE single-leg veto** (a healthy `dex_vol` reads HIBERNATE when `market:migration_count_1h` is starved — `grad_rate=0` forces it); `AGGRESSIVE_PAPER_TRADING` bypasses HIBERNATE for paper **AND** live (gate is in signal_aggregator, TEST_MODE-independent; the flip is bot_core-only → **live trades in HIBERNATE too**, so there is no paper-vs-live regime-eligibility gap); `market:mode:override` 24h-TTL renewal; and that a "market lull"/"no signals" condition is often a **pubsub-crash-loop pipeline OUTAGE** (check `portfolio_snapshots` heartbeat continuity, not just trade count). | Any V5A flip go/no-go turning on `market:mode:current`; any read of market_health mode; any paper→live edge projection involving regime; any `MARKET-MODE-001` recalibration; any "market lull"/"no signals"/low-new-token investigation; any reliance on governance as a HIBERNATE backstop. |

| `docs/audits/FULL_CODE_AUDIT_001_2026_06_02.md` (audit, not a findings doc — listed here for go-live discoverability) | Comprehensive read-only pre-flip codebase audit (~90 findings; 🔴×14). **§B is the canonical pre-flip remediation sequence.** Verified NON-findings: TEST_MODE money-path gating is correct/defense-in-depth (no real send in paper); private key not leaked. NEW 🔴 execution blockers: failed live sells booked as closed (D02-F1); partial/staged-TP live sell dumps the whole position (D02-F5); buy double-submit on confirm timeout (D02-F3); emergency_stop unreliable (D03-F1). Extends the pubsub outage to 5 services + the `main.py` no-supervisor amplifier; outage is invisible (heartbeats unread, no internal-service dashboard rows, liveness alerter undeployed). Fail-opens: governance (BUG-010), AGGRESSIVE_PAPER HIBERNATE bypass, daily-loss reset-on-restart, phantom `MAX_SD_POSITIONS`, dead `market:loss_override`. | Any V5A go/no-go or `TEST_MODE=false` flip; ANY change to the live execution path (`execution.py`, bot_core close/sell branch), any safety gate, any position-sizing or accounting code, any pubsub/gather/observability work. Use §B to sequence fix-sessions; each finding ID maps to a recommended fix. **Remediation progress is recorded in the companion `docs/audits/REMEDIATION_PHASE_0_1_2026_06_03.md`** (single oversight record: Phases 0+1+2+3 ALL DONE+deploy-verified as of 2026-06-03 (Phase-2 safety rails: #9/#10/#11+#12/#13; Phase-3 accounting: #14 live staged-TP cumulative PnL+Path-B multi-exit+on-chain reconcile `b6c52fc`, #15 dashboard mode-fidelity+entry-sentinel+DASH-CORRECTED-PNL-COLUMN-001 `888e2f5` incl. prod migration 003 adding corrected_* to `trades`, + NEW API-STATS-FSTRING-BUG-001 fixed `89ca0a1`); each fix's change/verification/runtime-status/pushed-commit/rollback in the oversight doc. **§B remediation is wholly code-complete + deploy-verified.** What's left before any `TEST_MODE=false` flip is NOT remediation code: 4 flagged decision-gated items (BUG-010 credits, SIZING-CAPS-WIRING-001 [MAX_SD_POSITIONS=20 deployed-but-unread], TIMEZONE-SIZING-FIX-001, GOVERNANCE-STALENESS-POLICY-001) + flip-time runtime confirmation of the live-only fixes + the live-flip preconditions below. **The durable go/no-go reference is `docs/audits/FLIP_READINESS_REVIEW_001_2026_06_03.md`** (FLIP-READINESS-REVIEW-001, 2026-06-03) — live-verified dependency/env/API matrix, the §6 flip-config (MAX_POSITION_SOL 0.25→0.10, DAILY_LOSS_LIMIT_SOL 4.0→1.5, AGGRESSIVE_PAPER_TRADING→false, concurrency 10→5), and the §7 runbook; read it before any flip session. |

| `docs/audits/LIVE_MC_CEILING_VERIFY_001_2026_06_08.md` (audit, not a findings doc — listed here for go-live discoverability) | Read-only verification of the live MC fill-ceiling vs paper's ~91% sub-$1k WR band — **the load-bearing live filter for the edge thesis.** Live MC gate is SAFE: fails CLOSED (`bot_core.py:1155-1157`), formula `price×1e9` USD **identical** across all 3 gates (SA `SD_MC_CEILING_USD=3000` signal-time fail-OPEN / paper-fill `BOT_CORE_FILL_MC_CEILING_USD=1000` / live-fill 1000), effective binding cap **$1000**, both ceilings **stable across the §6 flip-config** (flip touches sizing/loss/aggressive/concurrency, NOT the MC ceilings). **BUT the 91% does NOT transfer cleanly:** paper's `market_cap_at_entry` == the gate quantity `entry_price×1e9` (verified exact), yet the price INPUT diverges (live = Redis **fill-time** PumpPortal-stream-first; paper = Jupiter/Gecko→**signal-time** BC fallback + a paper-only slippage bump) → the live-admitted set = the paper band **± un-measurable signal→fill drift**, concentrated on a near-vertical **$1000 WR cliff**. **🟠 PROXY-ARTIFACT:** "sub-$1k" is a **price proxy**, not a true MC (true full-supply MC ~8× larger, median ~$5172); WR is a step-function of the proxy (`<500`=99.85% → `800-1000`=26.72% → `≥1000`≈0%); the 91% is the gate **selecting the winning side of its own proxy** (an entry-price-selection artifact), **NOT a validated small-MC edge** — treat as a paper-model hypothesis, an instance of `COST_FIDELITY_GAP`. Verified non-issue: 🔴 FAIL-OPEN does NOT fire (live fails closed); D08-F7's historical fail-open was patched by `c70aba1`. | Any V5A go/no-go or `TEST_MODE=false` flip; any "paper→live edge" / sizing / personality-activation projection leaning on the sub-$1k WR; any change to the MC gates (`SD_MC_CEILING_USD`, `BOT_CORE_FILL_MC_CEILING_USD`) or the `_get_token_price` pricing pipeline; any ML/threshold retune citing the sub-$1k band. |

| `docs/audits/EDGE_PROXY_ARTIFACT_EVAL_001_2026_06_09.md` (audit; go-live discoverability) | 🔴 **EDGE-INVALID.** The sub-$1k ~91% WR / +189.79 SOL that the live thesis rests on is **essentially 100% a paper artifact** — NOT a validated edge and does NOT transfer live. Mechanism: `paper_buy` prices entry via `paper_trader._get_token_price` (Jupiter/Gecko), which prices fresh pre-grad pump.fun **~10× below true MC** (100% of sub-$1k winners have `entry_price` below the physical BC floor 3e-6 = impossible fills); the deflated `entry_price` denominator makes a normal wiggle read as **peak/entry ~3.9×**, tripping the **+200% staged TP** (`bot_core.py:2152-2170`) to bank a fabricated win while the **final exit is below entry** (exit/entry median 0.863; single-exit PnL identity breaks on 2656/2657 winners). WR is a step-function of `entry_price` (breakpoint 1e-6), **not** of true MC; **de-artifacted WR 0–1%** (entry≥3e-6 → 0.00%); ML gate adds zero value. **Live does NOT reproduce** (live entry+exit share the Redis-stream oracle; live PnL = Path-B on-chain lamports, not the entry ratio). **ML corpus contaminated** (paper `pnl_sol` → `trades` table). Root cause: `paper_buy` prefers Jupiter over the correct BC-reserves price (`bot_core.py:1028-1033`, only a ≤0 fallback). Confirms + sharpens the parent `LIVE_MC_CEILING_VERIFY_001` 🟠 PROXY-ARTIFACT. | Any V5A go/no-go or `TEST_MODE=false` flip; ANY "paper→live edge"/sizing/personality-activation claim citing the sub-$1k WR (it is invalid); any ML retrain (corpus contaminated); any change to `paper_buy`/`_get_token_price`; any read of paper `realised_pnl_sol` as an edge signal. |

| `docs/audits/PAPER_ENTRY_ORACLE_FIX_001_2026_06_09.md` + `docs/audits/EDGE_RESEARCH_001_2026_06_09.md` (audits; go-live discoverability) | **The EDGE-PROXY root cause is FIXED + the edge path is mapped.** Fix (commit `419d1bc`, `T_fix=1781009204`, deployed+validated): pre-grad paper entries now price off **BC reserves** (`paper_trader.py:233-237`); env `BOT_CORE_FILL_MC_CEILING_USD=0`. **Validated: honest paper WR 92%→0.00%** — the strategy, honestly priced, is a loser (all fresh closes exit `no_momentum_90s`); the ARTIFACT CAVEAT now applies only to paper rows with `entry_time < 1781009204`. **EDGE-RESEARCH-001 verdict:** NO silver-bullet paid edge — recoverable edge is overwhelmingly **FREE** (internal + execution vs the 25.8% cost); recommended paid spend ~$0-5/mo; **Nansen pre-grad smart-money is structurally DEAD** (retroactive labels; per-signal = 5× over the 10k/mo budget) → offline/post-grad only. Roadmap (each paper→supervised-live): `FEATURE-PERSIST-001` (keystone) → `ML-FLOOR-RETUNE-003` → `NO-MOMENTUM-90S-TUNE-001` (−22 SOL bucket) → `GRAD-BYPASS-FIX-001` (−15 SOL) → cheap Helius on-chain → selective post-grad. **Before ANY flip:** `MC-CEILING-TRUE-SCALE-REDECIDE-001` (fill ceiling is at 0) + `ML-CORPUS-QUARANTINE-CUTOFF-001` (the `trades` ML table is poisoned by pre-fix artifact labels). | Any V5A flip; any edge/sizing/personality claim; any ML retrain; any paper→live projection; any work on `paper_buy` pricing / the BC-primary path; any data-source/MCP/credit decision (esp. Nansen). |
...
> **⚠️ ARTIFACT CAVEAT (EDGE-PROXY-ARTIFACT-EVAL-001 → FIXED by PAPER-ENTRY-ORACLE-FIX-001, 2026-06-09):** for **paper pre-grad SD rows with `entry_time < 1781009204`** (before commit `419d1bc`), `realised_pnl_sol` is **artifact-inflated** and is **NOT a valid edge/WR signal** — paper entry was priced via Jupiter/Gecko ~10× below true MC, so the `entry_price` denominator fabricated staged-TP "wins" (the ~92% sub-$1k WR is ~100% artifact; de-artifacted 0–1%, **confirmed live at 0% WR post-fix**). **Rows with `entry_time ≥ 1781009204` are HONEST** (entry now prices off BC reserves). Still: **do not derive a live-edge claim from paper PnL** — only live Path-B `corrected_pnl_sol` (on-chain lamports) validates an edge (`COST_FIDELITY_GAP`). See `docs/audits/PAPER_ENTRY_ORACLE_FIX_001_2026_06_09.md` + `EDGE_PROXY_ARTIFACT_EVAL_001_2026_06_09.md`. Column-provenance mechanics below are still accurate.
...
| 80+ | 172 | 52.9% | +57.5 SOL |
  ...
  **2026-05-01 update (STATE-RECONCILE-2026-05-01).** The 2026-04-17/19 magnitudes have collapsed by ~50-100× in the recent 7d sample. Current 7d SD-paper (n=689): 30-40 +0.49 / 40-50 **-1.98 SOL (worst)** / 50-60 -0.18 / 60-70 -0.70 / 70-80 **+1.63 (best)** / 80-90 -0.35 / 90+ +0.005. "Higher scores are better" now holds only weakly — only 70-80 reliably positive in the sample, while 80-90 is mildly negative. **The 2026-04-17/19 block remains a historical reference**; current cutoff guidance is uncertain pending `ML-THRESHOLD-DATA-DRIVEN-RETUNE-002` (re-queue ≥2026-05-12 post-this-deploy). See `docs/audits/USERMEMORIES_DRIFT_2026_05_01.md` §1A for full data and `.tmp_state_reconcile/findings_A_through_E.txt` for raw SQL output.

  **2026-05-05 update (BOT-CORE-ML-GATE-001).** ML threshold is now controlled by **two env vars** in paper mode. `ML_THRESHOLD_SPEED_DEMON` is the SA-side gate (overridden to 30 in paper by AGGRESSIVE_PAPER+TEST_MODE per `signal_aggregator.py:158-160`; this is intentional for ML training data composition — SA still scores 30-40 signals so the model has labeled-outcome breadth). `ML_THRESHOLD_BOT_CORE_SD` is the bot_core-side gate (default 0 = disabled; set on Railway to non-zero to activate; currently 40, env-active 2026-05-05T14:16:48Z UTC). Both gates are active simultaneously; the more restrictive one binds. **The bot_core gate is the only paper-effective filter on whether a scored signal becomes a paper trade.** All Session 4 / ML-RETUNE-001 sweep findings ("optimum=55") were derived from a contaminated sample (gate didn't exist) and should be re-derived post-this-deploy after ≥7d of post-gate accumulation. Previous claims that env-var changes to `ML_THRESHOLD_SPEED_DEMON` would affect paper sample are obsolete — that path was effectively dead code in paper mode prior to this session.
...
- **Concurrency caps are TWO-LAYERED (SIZING-CAPS-WIRING-001 + -B, 2026-06-03/04):** (1) TOTAL cross-personality cap at `bot_core.py:831` = `min(MAX_CONCURRENT_POSITIONS env, gov.max_concurrent_positions)` — env is a hard ceiling, governance can only tighten. (2) PER-PERSONALITY cap at `risk_manager.py:51` = `int(os.getenv("MAX_CONCURRENT_PER_PERSONALITY", "3"))` (WHALE env default 2), enforced in `calculate_position_size`→0.0→`bot_core:898`. **The per-personality cap BINDS FIRST for single-personality (SD-only) operation, so effective SD concurrency = `min(MAX_CONCURRENT_PER_PERSONALITY, MAX_CONCURRENT_POSITIONS)`.** Both are env-wired; currently **both set to 10 → effective 10**. `MAX_SD_POSITIONS` (env 20) is phantom (read by nothing) — do NOT use it; the live lever is `MAX_CONCURRENT_PER_PERSONALITY`.
...
**Canonical pre-flight gate (FLIP-NIGHT-PREP-001, 2026-06-03):** run **`python scripts/flip_preflight_check.py`** — a read-only GREEN/YELLOW/RED verifier of env (bot_core + signal_aggregator), Redis safety keys, on-chain balance, open-position count, and API reachability. Require **ALL-GREEN except `TEST_MODE`** (which flips LAST) before setting `TEST_MODE=false`. The one-command rollback is **`bash scripts/flip_rollback.sh`** (CONFIRM-gated; restores pre-flip config; never touches `bot:emergency_stop`). Verified safe: #9 HIBERNATE veto gates ENTRIES only — exits run regardless of mode, so a HIBERNATE dip during the watch is NOT a rollback trigger (`docs/audits/FLIP_NIGHT_PREP_001_2026_06_03.md` Part A). **The full operator runbook is `docs/FLIP_NIGHT_PLAYBOOK.md`** (STEP 0–9; the flip itself is operator-executed + session-gated — CC does not auto-flip). The CLEAN-003 steps below remain the Redis-cleanup substep (PLAYBOOK Step 1).
