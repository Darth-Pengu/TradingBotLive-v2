...
> **⚠️ ARTIFACT CAVEAT (EDGE-PROXY-ARTIFACT-EVAL-001, 2026-06-09):** for **paper pre-grad SD rows**, `realised_pnl_sol` is **artifact-inflated** and is **NOT a valid edge/WR signal.** Paper entry is priced via Jupiter/Gecko ~10× below true MC, so the `entry_price` denominator fabricates staged-TP "wins" (sub-$1k WR ~92% is ~100% artifact; de-artifacted 0–1%). Use `realised_pnl_sol` for accounting/bookkeeping, but **do not derive an edge claim from paper PnL** — only live Path-B `corrected_pnl_sol` (on-chain lamports) validates an edge. See `docs/audits/EDGE_PROXY_ARTIFACT_EVAL_001_2026_06_09.md`. The mechanics below (column provenance) are still accurate.
