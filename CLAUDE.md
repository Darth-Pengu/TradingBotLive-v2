...
- **Concurrency caps are TWO-LAYERED (SIZING-CAPS-WIRING-001, 2026-06-03) — footgun:** (1) TOTAL cross-personality cap at `bot_core.py:831` = `min(MAX_CONCURRENT_POSITIONS env [=10], gov.max_concurrent_positions)` — env is a hard ceiling, governance can only tighten. (2) PER-PERSONALITY cap at `risk_manager.py:51 MAX_CONCURRENT_PER_PERSONALITY=3` (WHALE=2), HARDCODED, enforced in `calculate_position_size`→0.0→`bot_core:898`. **The per-personality 3 BINDS FIRST for single-personality (SD-only) operation — the effective cap is 3, not 10.** `MAX_SD_POSITIONS` (env 20) is phantom (read by nothing). To change the *effective* SD trial cap, wire `MAX_CONCURRENT_PER_PERSONALITY` (SIZING-CAPS-WIRING-001-B), NOT `MAX_CONCURRENT_POSITIONS`/`MAX_SD_POSITIONS`.
