...
3. **Session 4** ✅ DONE — Tier 1 trading-tune env-var bundle (LIVE-001) deployed; 24h observation clock running.
4. **Session 2b** ✅ DONE — DASH-ENTRY-001 live entry INSERT to paper_trades (commit `dafe1fa`).
5. **Session 3** ✅ DONE — PumpPortal Local spec audit (Verdict B: spec matches code). Devnet Jupiter cycle blocked on faucet exhaustion + structural URL hardcoding; followed plan's fallback branch. No code change. See `docs/audits/SELL_PATH_DEVNET_VALIDATION_2026_04_19.md`.
6. **SEC-001** 📋 parallel (manual, Jay) — rotate Redis + Postgres passwords.
7. **Session 5** 🎯 supervised live-enable (LIVE-002). Gated on Session 4's 24h observation completion + SEC-001. Session 3 cleared it with conditions (tightened sell-side error budget + Sentry body capture on first HTTP 400).
...
| LIVE-002 | Supervised live-enable session (Steps A-I from `ZMN_LIVE_ENABLE_HANDOFF_2026_04_19.md`). **Success criterion amended 2026-04-19 post-Session 2 v2:** "Dashboard Recent Trades + aggregates + Treasury show accurate data for all 3+ live trades. **Open Positions widget for live mode is gated on DASH-ENTRY-001** — if DASH-ENTRY-001 has NOT landed, Open Positions for live will be empty during the live window and monitoring falls back to Railway logs + direct DB queries." **Success criterion further amended 2026-04-20:** "**Zero trapped positions** — every live-opened trade during the window reaches `exit_time IS NOT NULL` within the 60-min window, OR within 5 min of window end if still mid-TP-schedule. Trapped positions (sell-route failures with no recovery) indicate EXEC-001 / EXEC-002 not deployed or incomplete." **v4 attempt outcome (2026-04-20):** ROLLBACK at T+12:00 on T5 trigger. Single live trade executed (0.365 SOL round-trip on mint `yh3n441...`). Net wallet impact: -0.094 SOL (5.68% of T0 1.658 SOL). Trigger fired on benign position-state not real loss — see T5-DESIGN-001. Other critical findings: reconcile-residual from paper positions (CLEAN-003 ✅ FIX-1), ML threshold bypass (ML-012), fee model 96× under-counting (FEE-MODEL-001 ✅ FIX-2). **Revised v5 success criteria** (additions to v4 criteria): (1) Paper model corrected per FEE-MODEL-001 with historical edge re-baselined (pre-flight gate — landed `e078b4c`); (2) Pre-flip Redis cleanup executed per CLEAN-003 (pre-flight gate — landed FIX-1); (3) ML-012 landed so threshold policy binds (pre-flight gate); (4) T5a/T5b new trigger definitions active (Session 5 v5 prompt per T5-DESIGN-001); (5) MAX_POSITION_SOL_FRACTION active in bot_core (pre-flight gate — FIX-1); (6) First live trade's on-chain delta within 2× of paper-model prediction (success criterion, not gate). | 📋 QUEUED (v5 pending) | 3 hours incl. 30 min obs | begins live operation | FEE-MODEL-001 ✅ + CLEAN-003 ✅ + ML-012 + T5-DESIGN-001 + Session 3 devnet | `ZMN_OPTIMIZATION_PLAN_2026_04_19.md` Tier 3 §3.7 + abort-report checklist; `SENTRY_TRIAGE_2026_04_19.md`; `ZMN_DASH_FIX_DONE.md`; `docs/audits/SESSION_5_V4_ROLLBACK_FINDINGS.md` |
