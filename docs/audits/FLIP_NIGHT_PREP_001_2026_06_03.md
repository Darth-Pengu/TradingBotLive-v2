...
3. **Effective concurrency** — ✅ **RESOLVED (SIZING-CAPS-WIRING-001-B landed 2026-06-04):** `MAX_CONCURRENT_PER_PERSONALITY` env-wired + set =10 on bot_core → effective SD concurrency = `min(10, MAX_CONCURRENT_POSITIONS=10)` = **10** (Jay's decision; was 3). The verifier now computes + reports the effective cap (GREEN at 10).
