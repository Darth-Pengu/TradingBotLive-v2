...
# MAX_CONCURRENT_POSITIONS=10       ← already set (SIZING-CAPS-WIRING-001); VERIFY, do not re-set
# MAX_CONCURRENT_PER_PERSONALITY=10 ← already set (SIZING-CAPS-WIRING-001-B); VERIFY → effective concurrency = 10
...
> **Concurrency (SIZING-CAPS-WIRING-001 + -B, both LANDED 2026-06-04):** effective SD concurrency = `min(MAX_CONCURRENT_PER_PERSONALITY=10, MAX_CONCURRENT_POSITIONS=10)` = **10**. Both env vars are set on bot_core; the per-personality cap (`risk_manager.py:51`) is now env-wired (was hardcoded 3). So the trial can hold **up to 10 concurrent positions** — bounded further by `MAX_WALLET_EXPOSURE=0.25` (25% of wallet) and per-position `MAX_POSITION_SOL=0.10`.
...
- `[CAPS] concurrency cap=10 (env=10, gov=10)` present (total cap; effective SD cap also 10 via MAX_CONCURRENT_PER_PERSONALITY=10).
...
- **Up to 10 concurrent positions** → the wired cap (per-personality=10, total=10). Bounded by wallet exposure (25%) + per-position size. Fewer than 10 is normal (signal flow).
